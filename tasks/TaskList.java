...
public class TaskList {
    ...
    public TaskList(List<Task> tasks) {
        ...
        if (tasks.size() == 0) {
            throw new IllegalArgumentException("Size of tasklist is 0");
        }
        for (int i = 0; i < tasks.size() - 1; i++) {
            if (tasks.get(i).getType() == TaskType.AWAY) {
                if (tasks.get(i + 1).getType() != TaskType.AWAY
                        && tasks.get(i + 1).getType() != TaskType.LAND) {
                    throw new IllegalArgumentException("Order invalid");
                }
            } else if (tasks.get(i).getType() == TaskType.LAND) {
                if (tasks.get(i + 1).getType() != TaskType.WAIT
                        && tasks.get(i + 1).getType() != TaskType.LOAD) {
                    throw new IllegalArgumentException("Order invalid");
                }
            } else if (tasks.get(i).getType() == TaskType.WAIT) {
                if (tasks.get(i + 1).getType() != TaskType.WAIT
                        && tasks.get(i + 1).getType() != TaskType.LOAD) {
                    throw new IllegalArgumentException("Order invalid");
                }
            } else if (tasks.get(i).getType() == TaskType.LOAD) {
                if (tasks.get(i + 1).getType() != TaskType.TAKEOFF) {
                    throw new IllegalArgumentException("Order invalid");
                }
            } else if (tasks.get(i).getType() == TaskType.TAKEOFF) {
                if (tasks.get(i + 1).getType() != TaskType.AWAY) {
                    throw new IllegalArgumentException("Order invalid");
                }
            }
     ...
     * between 1 and N inclusive, where N is the number of tasks in the task list and encodedTask1
