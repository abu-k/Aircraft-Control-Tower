...
public class ViewModel {
    ...
    public ViewModel(List<String> filenames) throws IOException, MalformedSaveException {
        ...
        if (tower.getTerminals() == null) {
            this.numTerminals.set(0);
        } else {
    ...
    public EventHandler<ActionEvent> getFindSuitableGateHandler() {
        return actionEvent -> {
            ...
            if (aircraft != null
                    && aircraft.getTaskList().getCurrentTask().getType().equals(TaskType.LAND)) {
                try {
                    gate = getControlTower().findUnoccupiedGate(aircraft);
    ...
    public void saveAs(Writer tickWriter, Writer aircraftWriter, Writer queuesWriter,
        ...
        // Writing to tickWriter
        BufferedWriter bufferedTickWriter = new BufferedWriter(tickWriter);
        bufferedTickWriter.write("" + getControlTower().getTicksElapsed());
        bufferedTickWriter.close();

        // Writing to aircraftWriter
        StringBuilder aircraftWriterStringBuilder;
        aircraftWriterStringBuilder = new StringBuilder();
        aircraftWriterStringBuilder.append(getControlTower().getAircraft().size()).append("\n");
        for (Aircraft aircraft : getControlTower().getAircraft()) {
            aircraftWriterStringBuilder.append(aircraft.encode()).append("\n");
        }
        BufferedWriter bufferedAircraftWriter = new BufferedWriter(aircraftWriter);
        bufferedAircraftWriter.write(aircraftWriterStringBuilder.toString().strip());
        bufferedAircraftWriter.close();

        // Writing to queuesWriter
        StringBuilder queuesWriterStringBuilder;
        queuesWriterStringBuilder = new StringBuilder();
        queuesWriterStringBuilder.append(getControlTower().getTakeoffQueue().encode()).append("\n");
        queuesWriterStringBuilder.append(getControlTower().getLandingQueue().encode()).append("\n");
        queuesWriterStringBuilder
                .append("LoadingAircraft:").append(getControlTower().getLoadingAircraft().size());
        for (Aircraft aircraft : getControlTower().getLoadingAircraft().keySet()) {
            queuesWriterStringBuilder.append(aircraft.getCallsign()).append(":")
                    .append(getControlTower().getLoadingAircraft().get(aircraft)).append("\n");
        }
        BufferedWriter bufferedQueuesWriter = new BufferedWriter(queuesWriter);
        bufferedQueuesWriter.write(queuesWriterStringBuilder.toString().strip());
        bufferedQueuesWriter.close();

        // Writing to terminalsWithGatesWriter
        StringBuilder terminalsWithGatesWriterStringBuilder;
        terminalsWithGatesWriterStringBuilder = new StringBuilder();
        terminalsWithGatesWriterStringBuilder.append(getControlTower().getTerminals().size())
                .append("\n");
        for (Terminal terminal : getControlTower().getTerminals()) {
            terminalsWithGatesWriterStringBuilder.append(terminal.encode());
        }
        BufferedWriter bufferedTerminalsWithGatesWriter =
                new BufferedWriter(terminalsWithGatesWriter);
        bufferedTerminalsWithGatesWriter.write(terminalsWithGatesWriterStringBuilder
                .toString().strip());
        bufferedTerminalsWithGatesWriter.close();
    }
